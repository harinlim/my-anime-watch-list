...
CREATE OR REPLACE FUNCTION auth.to_jsonb2(anyelement text)
 RETURNS jsonb
 LANGUAGE sql
AS $function$
...
begin
  ...
  fields_match := (select exists (
    select 1 from auth.users
    where id = new.id
      and (email = new.email or raw_user_meta_data->'email' = new_email)
...
CREATE OR REPLACE FUNCTION public.to_jsonb2(anyelement text)
 RETURNS jsonb
 LANGUAGE sql
AS $function$
SELECT COALESCE(to_jsonb(anyelement), jsonb 'null')
